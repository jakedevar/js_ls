...
--------------------- Problem ----------------------
write a function that implements a stack and register program, 
it takes a string that represents different commands and uses them to feed into the program which then preforms different operations on a stack and a register

------------ **Explicit Requirements** -------------
-  Rules:
  - all operations are integer operations though are fed as strings
  - programs will be fed by a string arugment
  - all programs are valid programs and wont have unexpected behavior
  - the stack will start as []
  - the register will start at 0
- Inputs: a string representing tokens to the program and integers
- Output: a number as a result of the operations
------------ **Implicit Requirements** -------------
- need to convert numbers that are strings to actual numbers
------------- **Clarifying Questions** -------------
- need to check for valid input?
------------ Examples/Test Cases/Edge's ------------
-  Visual Reference: given
------------------ Data Structure ------------------
- split string with split spaces
...
function minilang(commands) {
  ...
  tokens.forEach(token => {
    switch (token) {
      case 'PUSH': stack.push(register) ;break;
      case 'ADD': register += stack.pop() ;break;
      case 'SUB': register -= stack.pop() ;break;
      case 'MULT': register *= stack.pop() ;break;
      case 'DIV': register = Math.floor(register / stack.pop()) ;break;
      case 'REMAINDER': register %= stack.pop() ;break;
      case 'POP': register = stack.pop() ;break;
      case 'PRINT': console.log(Math.floor(register)) ;break;
