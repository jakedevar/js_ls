...
---------------------- Notes -----------------------
so running into some trouble that i didnt expect from last time, the periods remain even after splitting with regex which occured last time
so it looks like that was part of my solution was silently failing. 
*/
...
function longestSentence(text) {
  let splitParagraph = text.match(/\w[^.!?]*?[.!?]/g)

  let longest = splitParagraph.reduce((longest, currentSentence) => {
    if (Len(longest) > Len(currentSentence)) {
      return longest;
    } else {
